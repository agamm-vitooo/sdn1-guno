        ...
        {prestasis.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {prestasis.map((p) => (
              <article
                key={p.id}
                className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border-2 border-transparent hover:border-yellow-400 transform hover:-translate-y-2"
              >
                {/* Image Container */}
                {p.image_url && (
                  <div className="relative w-full h-64 bg-gradient-to-br from-yellow-100 to-orange-100 overflow-hidden">
                    <Image
                      src={p.image_url}
                      alt={p.title}
                      fill
                ...
                {/* Content */}
                <div className="p-6">
                  <h2 className="text-xl font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-orange-600 transition-colors duration-200">
                    {p.title}
                  </h2>
