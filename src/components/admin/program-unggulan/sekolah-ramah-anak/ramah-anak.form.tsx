...
interface Props {
  editItem?: SekolahRamahAnak | null;
  onSuccess: () => void;
  onCancel?: () => void;
}

export default function FormSekolahRamahAnak({
  editItem,
  onSuccess,
  onCancel,
}: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (maxReached && !editItem) {
      alert("Data hanya boleh 1! Silakan edit data yang sudah ada.");
      return;
    }
    ...
    if (editItem) {
      ...
      await supabase.from("sekolah_ramah_anak").insert([{ title, description }]);
    }
